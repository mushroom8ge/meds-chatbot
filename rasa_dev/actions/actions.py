...
def _create_viewer_path(patient_id: Text, medication_status: Text) -> Text:
    """Creates a path to a specific patient's medicine history."""
    if medication_status in ["preparation", "in-progress", "cancelled", "on-hold", "completed", "entered-in-error","stopped", "declined", "unknown", "active"]:
        return ENDPOINT["status"].format(patient_id, medication_status)
    return ENDPOINT["base"].format(patient_id)
...
class ActionFindMedication(Action):
    """This action class retrieves the medication of the user in specific status
     to display it to the user."""

    def name(self) -> Text:
        """Unique identifier of the action"""

        return "action_find_medication"
